...
exports.run = async (client, message, args) => {
  ...
  if (message.author.id !== '310362541807370255') return message.channel.send(çarpı + " Bu komutu sadece `Yapımcım` **kullanabilir!**")
  let nesne = args[0]
 ...
 await db.set(`gold_${nesne}`, 'tm')
  ...
  message.channel.send(`${tik} <@${nesne}> kullanıcısı yani **${nesne}** IDli kullanıcı gold üye oldu!`)
   client.guilds.get("310362541807370255").channels.find(x => x.name === "➵gold-üye-log").send(`${tik} <@${nesne}> kullanıcısı için gold üyeliği **açıldı!**`)
