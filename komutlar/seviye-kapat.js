...
exports.run = async(client, message, args) => { 
 ...
 let hm = await db.fetch(`seviyeacik_${message.guild.id}`)
  ...
  let rollvl = await db.fetch(`rollevel_${message.guild.id}`)
    ...
    if (!message.member.hasPermission("ADMINISTRATOR")) return message.channel.send(` <a:dikkat:310362541807370255> Bu komutu kullanabilmek için "\`Yönetici\`" yetkisine sahip olmalısın.`);
